...
export default function EscandallosView() {
  ...
  const handleCreateRecipe = () => {
    ...
    setNewRecipe({ name: '', category: 'Entrantes', servings: 1, margin: 75, laborCost: 2.50, ingredients: [] });
  ...
  const addIngredientToNewRecipe = () => {
    const newIng: RecipeIngredient = {
      id: Math.random().toString(36).substr(2, 9),
      name: '', quantity: 0, unit: 'kg', pricePerUnit: 0, wastePercentage: 0
    };
    setNewRecipe(prev => ({ ...prev, ingredients: [...prev.ingredients, newIng] }));
  };
  ...
  const handleUpdateRecipeIngredients = (updatedIngredients: RecipeIngredient[]) => {
    setRecipes(prev => prev.map(r => r.id === selectedRecipeId ? { ...r, ingredients: updatedIngredients } : r));
  ...
  const handleOptimize = async (isGlobal: boolean = false) => {
    ...
    try {
      ...
      setAiOptimizationResult({
        recommendations: result.recommendations || ["Reducir la merma mediante un corte más preciso", "Ajustar el gramaje de la guarnición", "Negociar volumen con el proveedor principal", "Optimizar el tiempo de preparación para reducir coste laboral"],
      ...
      setAiOptimizationResult({
        recommendations: ["Error al conectar con la IA. Usando sugerencias predefinidas.", "Reducir la merma mediante un corte más preciso", "Ajustar el gramaje de la guarnición", "Optimizar el tiempo de preparación"],
    ...
    setTimeout(() => {
      if (aiOptimizationResult) {
        if (aiOptimizationResult.isGlobal) {
          setRecipes(prev => prev.map(r => ({ ...r, margin: Math.min(90, r.margin + 2) })));
        } else {
          setRecipes(prev => prev.map(r => {
            if (r.id === selectedRecipeId) {
              return { ...r, margin: Math.min(90, r.margin + 5), laborCost: Math.max(0.5, r.laborCost * 0.9) };
            }
            return r;
          }));
        }
      ...
      setIsOptimizing(false);
      showToast('Optimizaciones aplicadas correctamente.', 'success');
    }, 1500);
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => setShowAdjustWasteModal(true)}
      ...
      <div className="flex gap-1.5 p-1.5 bg-slate-100 rounded-2xl w-fit">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
            className={cn("flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black transition-all uppercase tracking-widest",
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'escandallo' && (
          <motion.div key="escandallo" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
                ...
                <div className="divide-y divide-slate-50 max-h-[600px] overflow-y-auto">
                  {recipes.length === 0 && (
                    <div className="p-8 text-center">
                      <p className="text-sm text-slate-400 font-bold uppercase tracking-widest">No hay recetas todavía</p>
                    </div>
                  )}
                  {recipes.map((recipe) => (
                    <button key={recipe.id} onClick={() => setSelectedRecipeId(recipe.id)}
                      className={cn("w-full flex items-center justify-between p-6 text-left transition-all group",
                        selectedRecipeId === recipe.id ? "bg-emerald-50" : "hover:bg-slate-50")}>
                      <div>
                        <p className={cn("font-black text-sm", selectedRecipeId === recipe.id ? "text-emerald-700" : "text-slate-700")}>{recipe.name}</p>
                      ...
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => { e.stopPropagation(); const duplicated = { ...recipe, id: Math.random().toString(36).substr(2, 9), name: `${recipe.name} (Copia)` }; setRecipes(prev => [duplicated, ...prev]); showToast('Receta duplicada'); }}
                          className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-xl transition-all">
                          <Copy size={16} />
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); if (confirm(`¿Estás seguro de eliminar ${recipe.name}?`)) { setRecipes(prev => prev.filter(r => r.id !== recipe.id)); if (selectedRecipeId === recipe.id) { setSelectedRecipeId(recipes.find(r => r.id !== recipe.id)?.id ?? null); } showToast('Receta eliminada'); } }}
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                    ...
                    <div className="space-y-6">
                      ...
                      <input type="range" min="10" max="90" value={simulatedMargin} onChange={(e) => setSimulatedMargin(parseInt(e.target.value))}
                      ...
                      <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                        ...
                        <div className="flex justify-between items-center text-[10px] text-indigo-500 font-bold">
                          ...
                          <span className={cn((calculateTotalCost(selectedRecipe) / (1 - simulatedMargin / 100)) > calculateRecommendedPrice(selectedRecipe) ? "text-emerald-600" : "text-rose-600")}>
                      ...
                      <div className="flex items-center gap-6 mt-3">
                        ...
                        <div className="flex items-center gap-1">
                          {['GL', 'LC', 'HS'].map(allergen => (
                            <span key={allergen} className="w-6 h-6 rounded-full bg-rose-50 text-rose-600 text-[8px] font-black flex items-center justify-center border border-rose-100">{allergen}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => setIsEditingIngredients(!isEditingIngredients)}
                        className={cn("p-3 rounded-2xl transition-all shadow-sm bg-white flex items-center gap-2 px-4",
                      ...
                      <button className="p-3 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all shadow-sm bg-white"><FileDown size={20} /></button>
                      <button className="p-3 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-2xl transition-all shadow-sm bg-white"><History size={20} /></button>
                      <button className="p-3 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-2xl transition-all shadow-sm bg-white"><MoreVertical size={20} /></button>
                    </div>
                  </div>
                  <div className="p-0">
                      ...
                      <tbody className="divide-y divide-slate-50">
                        <AnimatePresence>
                          {(selectedRecipe?.ingredients ?? []).map((ing, index) => {
                            ...
                            return (
                              <motion.tr key={ing.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }}
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      {isEditingIngredients ? (
                                        <input type="text" className="text-sm font-black text-slate-700 bg-transparent border-b border-slate-200 focus:border-emerald-500 outline-none"
                                          value={ing.name} onChange={(e) => { const updated = [...(selectedRecipe?.ingredients ?? [])]; updated[index] = { ...updated[index], name: e.target.value }; handleUpdateRecipeIngredients(updated); }} />
                                      ) : (
                                ...
                                <td className="px-6 py-6 text-sm font-mono text-slate-500 text-center">
                                  {isEditingIngredients ? (
                                    <div className="flex items-center justify-center gap-1">
                                      <input type="number" className="w-16 text-center bg-transparent border-b border-slate-200 focus:border-emerald-500 outline-none"
                                        value={ing.quantity} onChange={(e) => { const updated = [...(selectedRecipe?.ingredients ?? [])]; updated[index] = { ...updated[index], quantity: parseFloat(e.target.value) || 0 }; handleUpdateRecipeIngredients(updated); }} />
                                      <span>{ing.unit}</span>
                                    </div>
                                ...
                                <td className="px-6 py-6 text-center">
                                  {isEditingIngredients ? (
                                    <input type="number" className="w-12 text-center bg-transparent border-b border-slate-200 focus:border-emerald-500 outline-none"
                                      value={ing.wastePercentage} onChange={(e) => { const updated = [...(selectedRecipe?.ingredients ?? [])]; updated[index] = { ...updated[index], wastePercentage: parseFloat(e.target.value) || 0 }; handleUpdateRecipeIngredients(updated); }} />
                                  ) : (
                                ...
                                {isEditingIngredients && (
                                  <td className="px-6 py-6 text-center">
                                    <button onClick={() => { const updated = (selectedRecipe?.ingredients ?? []).filter((_, i) => i !== index); handleUpdateRecipeIngredients(updated); }}
                        ...
                        {isEditingIngredients && (
                          <tr>
                            <td colSpan={6} className="px-10 py-4">
                              <button onClick={() => { const newIng: RecipeIngredient = { id: Math.random().toString(36).substr(2, 9), name: 'Nuevo Ingrediente', quantity: 0, unit: 'kg', pricePerUnit: 1, wastePercentage: 0 }; handleUpdateRecipeIngredients([...(selectedRecipe?.ingredients ?? []), newIng]); }}
                        ...
                        <tr className="bg-slate-50/30">
                          ...
                          <td className="px-10 py-6 text-sm font-black text-slate-900 text-right">
                            {isEditingIngredients ? (
                              <input type="number" step="0.01" className="w-24 text-right bg-transparent border-b border-slate-200 focus:border-emerald-500 outline-none"
                                value={selectedRecipe.laborCost} onChange={(e) => { setRecipes(prev => prev.map(r => r.id === selectedRecipeId ? { ...r, laborCost: parseFloat(e.target.value) || 0 } : r)); }} />
                    ...
                    <div className="bg-rose-50 p-6 rounded-[2rem] border border-rose-100 shadow-sm">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="pt-4 border-t border-rose-100">
                          <button onClick={() => setActiveTab('mermas')} className="w-full py-3 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all">
                            Ver Detalle Mermas
                          </button>
                        </div>
                      </div>
        ...
        {activeTab === 'analisis' && (
          <motion.div key="analisis" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
            ...
            <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm p-12">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase mb-12">Matriz de Ingeniería de Menú</h3>
                  ...
                  <div className="absolute inset-0 grid grid-cols-2 grid-rows-2">
                    <div className="border-r border-b border-slate-200 flex flex-col items-center justify-center p-8 bg-emerald-50/30">
                      ...
                      <div className="flex flex-wrap gap-2 justify-center">
                        {[1,2,3].map(i => <div key={i} className="w-4 h-4 bg-emerald-500 rounded-full shadow-sm shadow-emerald-200" />)}
                      </div>
                    </div>
                    ...
                    <div className="border-r border-slate-200 flex flex-col items-center justify-center p-8 bg-amber-50/30">
                      ...
                      <div className="flex flex-wrap gap-2 justify-center">
                        {[1,2].map(i => <div key={i} className="w-4 h-4 bg-amber-500 rounded-full shadow-sm shadow-amber-200" />)}
                      </div>
                    </div>
                ...
                <div className="space-y-8">
                  ...
                  <div style={{ minHeight: 256 }}>
                    <ResponsiveContainer width="100%" height={256}>
                      <LineChart data={[{ name: 'Ene', cost: 2.4 }, { name: 'Feb', cost: 2.6 }, { name: 'Mar', cost: 2.5 }, { name: 'Abr', cost: 2.8 }, { name: 'May', cost: 3.1 }, { name: 'Jun', cost: 2.9 }]}>
        ...
        {activeTab === 'optimizacion' && (
          <motion.div key="optimizacion" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="space-y-8">
            <div className="bg-slate-900 rounded-[3rem] p-12 text-white relative overflow-hidden">
              <div className="absolute top-0 right-0 p-12 opacity-10"><Brain size={160} /></div>
              <div className="relative z-10 max-w-2xl">
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-14 h-14 bg-indigo-500 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-500/20"><Brain size={32} /></div>
                  <div>
                ...
                <p className="text-slate-400 text-lg font-medium leading-relaxed mb-10">
                  Nuestra inteligencia artificial analiza tus escandallos comparándolos con el histórico de ventas, mermas registradas y tendencias de mercado.
                </p>
                <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                  <button onClick={() => handleOptimize(false)} disabled={isOptimizing}
                    ...
                    {isOptimizing && !aiOptimizationResult?.isGlobal ? (<><div className="w-4 h-4 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin" />Analizando...</>) : (<><Zap size={18} className="text-amber-500" />Analizar Plato Actual</>)}
                  </button>
                  <button onClick={() => handleOptimize(true)} disabled={isOptimizing}
                    ...
                    {isOptimizing && aiOptimizationResult?.isGlobal ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Análisis Global...</>) : (<><Sparkles size={18} className="text-amber-300" />Análisis Estratégico Global</>)}
                  </button>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-8">
                <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm p-10">
                  <h4 className="text-sm font-black uppercase tracking-widest text-slate-900 mb-8 flex items-center gap-2">
                    ...
                    {aiOptimizationResult?.isGlobal ? 'Estrategia Global' : 'Recomendaciones'}
                    {aiOptimizationResult && <span className="text-xs font-bold text-emerald-600 ml-auto">Análisis Completado</span>}
                  </h4>
                  <div className="space-y-6">
                    {aiOptimizationResult ? aiOptimizationResult.recommendations.map((rec, i) => (
                      <div key={i} className="flex items-start gap-6 p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 hover:bg-white hover:shadow-md transition-all group">
                        ...
                        <div className="flex-1">
                          <h5 className="text-base font-black text-slate-900 mb-2">{rec}</h5>
                          <p className="text-sm text-slate-500 font-medium leading-relaxed">Sugerencia generada por IA para optimizar la rentabilidad.</p>
                      ...
                      <div className="text-center py-12">
                        <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4"><Brain size={32} className="text-slate-300" /></div>
                  ...
                  {aiOptimizationResult?.marketTrend && (
                    <div className="mt-8 p-6 bg-amber-50 rounded-[2rem] border border-amber-100">
                      <h5 className="text-[10px] font-black uppercase tracking-widest text-amber-700 mb-2 flex items-center gap-2"><TrendingUp size={14} />Tendencia de Mercado</h5>
                      <p className="text-sm text-amber-900 font-medium leading-relaxed italic">"{aiOptimizationResult.marketTrend}"</p>
                    </div>
                  )}
                </div>
              </div>
              <div className="space-y-8">
                <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm p-10">
                  <h4 className="text-sm font-black uppercase tracking-widest text-slate-900 mb-8 flex items-center gap-2"><BarChart size={18} className="text-indigo-600" />Perfil de Optimización</h4>
        ...
        {activeTab === 'mermas' && (
          <motion.div key="mermas" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                { label: 'Total Mermas Mes', value: '1.240,50€', icon: TrendingDown, color: 'text-rose-600', bg: 'bg-rose-50' },
                { label: 'Impacto en Margen', value: '-4.2%', icon: AlertCircle, color: 'text-amber-600', bg: 'bg-amber-50' },
                { label: 'Plato más Afectado', value: 'Lubina a la Sal', icon: ChefHat, color: 'text-indigo-600', bg: 'bg-indigo-50' },
                { label: 'Eficiencia de Stock', value: '92.8%', icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-50' },
              ].map((stat, i) => (
                <div key={i} className={cn("p-8 rounded-[2.5rem] border border-transparent shadow-sm flex flex-col items-center text-center", stat.bg)}>
                  <div className={cn("p-4 rounded-2xl mb-4 bg-white shadow-sm", stat.color)}><stat.icon size={24} /></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 bg-white rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-10 border-b border-slate-100 flex items-center justify-between">
                  ...
                  <button onClick={() => setShowMermasModal(true)}
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50/50 border-b border-slate-100">
                        ...
                        <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Ingrediente</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {wasteRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-slate-50/30 even:bg-slate-50/10 transition-colors">
                          ...
                          <td className="px-6 py-6">
                            <span className={cn("text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest",
                              record.reason === 'Caducidad' ? "bg-rose-50 text-rose-600" : record.reason === 'Deterioro' ? "bg-amber-50 text-amber-600" : "bg-indigo-50 text-indigo-600")}>
                              {record.reason}
                            </span>
                          </td>
              ...
              <div className="space-y-8">
                <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm p-10">
                  <h4 className="text-sm font-black uppercase tracking-widest text-slate-900 mb-8 flex items-center gap-2"><PieChart size={18} className="text-rose-600" />Motivos de Merma</h4>
                  <div style={{ minHeight: 300 }}>
                    <ResponsiveContainer width="100%" height={300}>
                      <RadarChart cx="50%" cy="50%" outerRadius="80%" data={[
        ...
        {activeTab === 'innovaciones' && (
          <motion.div key="innovaciones" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-10">
            <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[3rem] p-16 text-white relative overflow-hidden shadow-2xl shadow-indigo-200">
              <div className="absolute top-0 right-0 p-16 opacity-10"><Zap size={200} /></div>
              <div className="relative z-10 max-w-3xl">
                ...
                <p className="text-indigo-100 text-lg font-medium leading-relaxed mb-10 opacity-90">
                  Explora las innovaciones tecnológicas que transformarán la rentabilidad y eficiencia de tu restaurante.
                </p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[
                { title: "Predicción de Demanda por IA", desc: "Algoritmos que analizan el clima, eventos locales y festivos para predecir cuántos clientes vendrán y qué pedirán.", icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50" },
                { title: "Escaneo de Mermas con Visión Artificial", desc: "Cámaras sobre los cubos de basura que identifican qué ingredientes se tiran más y sugieren ajustes.", icon: Eye, color: "text-rose-600", bg: "bg-rose-50" },
                { title: "Gestión de Precios Dinámicos", desc: "Ajuste automático de precios en cartas digitales según la demanda, hora del día o stock disponible.", icon: Zap, color: "text-amber-600", bg: "bg-amber-50" },
                { title: "Integración IoT en Cámaras", desc: "Sensores de temperatura y humedad conectados que alertan antes de que un producto se eche a perder.", icon: Package, color: "text-blue-600", bg: "bg-blue-50" },
                { title: "Optimización de Turnos por IA", desc: "Asignación automática de personal basada en la carga de trabajo prevista, reduciendo costes innecesarios.", icon: Clock, color: "text-blue-600", bg: "bg-blue-50" },
                { title: "Análisis de Sentimiento en Reseñas", desc: "Monitorización automática de Google y TripAdvisor para detectar problemas recurrentes y sugerir mejoras.", icon: MessageSquare, color: "text-rose-600", bg: "bg-rose-50" },
              ].map((item, i) => (
            ...
            <div className="p-10 space-y-8">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  ...
                  <div className="relative">
                    <input type="text" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-emerald-500 transition-all pr-14"
                      placeholder="Ej: Lubina a la Sal" value={newRecipe.name} onChange={e => setNewRecipe({...newRecipe, name: e.target.value})} />
                    <button onClick={startVoiceInput}
                      className={cn("absolute right-2 top-1/2 -translate-y-1/2 p-3 rounded-xl transition-all", isRecording ? "bg-rose-100 text-rose-600 animate-pulse" : "bg-white text-slate-400 hover:text-indigo-600 shadow-sm")}>
                ...
                <div>
                  ...
                  <select className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-emerald-500 transition-all"
                ...
                <div>
                  ...
                  <input type="number" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-emerald-500 transition-all"
                    value={newRecipe.servings} onChange={e => setNewRecipe({...newRecipe, servings: parseInt(e.target.value) || 1})} />
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Margen Objetivo (%)</label>
                  <input type="number" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-emerald-500 transition-all"
                    value={newRecipe.margin} onChange={e => setNewRecipe({...newRecipe, margin: parseInt(e.target.value) || 0})} />
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Coste Mano de Obra (€)</label>
                  <input type="number" step="0.01" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-emerald-500 transition-all"
                    value={newRecipe.laborCost} onChange={e => setNewRecipe({...newRecipe, laborCost: parseFloat(e.target.value) || 0})} />
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ingredientes</h4>
                  <button onClick={addIngredientToNewRecipe} className="text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:underline flex items-center gap-1">
                    <Plus size={14} /> Añadir
                  </button>
                </div>
                  ...
                  {newRecipe.ingredients.map((ing, idx) => (
                    <div key={ing.id} className="grid grid-cols-12 gap-3 items-center bg-slate-50 p-3 rounded-xl">
                      <div className="col-span-6">
                        <input type="text" placeholder="Ingrediente" className="w-full bg-transparent border-none text-xs font-bold focus:ring-0"
                          value={ing.name} onChange={e => updateNewRecipeIngredient(idx, 'name', e.target.value)} />
                      </div>
                      <div className="col-span-3">
                        <input type="number" placeholder="Cant" className="w-full bg-transparent border-none text-xs font-bold focus:ring-0 text-center"
                          value={ing.quantity} onChange={e => updateNewRecipeIngredient(idx, 'quantity', parseFloat(e.target.value) || 0)} />
                      </div>
                      <div className="col-span-2">
                        <select className="w-full bg-transparent border-none text-[10px] font-bold focus:ring-0" value={ing.unit} onChange={e => updateNewRecipeIngredient(idx, 'unit', e.target.value)}>
                      ...
                      <div className="col-span-1 flex justify-end">
                        <button onClick={() => removeIngredientFromNewRecipe(idx)} className="text-rose-400 hover:text-rose-600"><Trash2 size={14} /></button>
                      </div>
                    </div>
                  ))}
              ...
              <div className="space-y-4">
                {(selectedRecipe?.ingredients ?? []).map((ing, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <div className="flex items-center gap-3">
                      <input type="number" className="w-20 px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-center"
                        value={ing.wastePercentage} onChange={(e) => { const updated = [...(selectedRecipe?.ingredients ?? [])]; updated[i] = { ...updated[i], wastePercentage: parseFloat(e.target.value) || 0 }; handleUpdateRecipeIngredients(updated); }} />
                      <span className="text-sm font-bold text-slate-500">%</span>
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={() => { setShowAdjustWasteModal(false); showToast('Mermas técnicas actualizadas correctamente'); }}
            ...
            <div className="p-10 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  ...
                  <select className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-rose-500 transition-all"
                    value={newWaste.dishId} onChange={e => setNewWaste({...newWaste, dishId: e.target.value})}>
                ...
                <div>
                  ...
                  <input type="date" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-rose-500 transition-all"
                    value={newWaste.date} onChange={e => setNewWaste({...newWaste, date: e.target.value})} />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <input type="text" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-rose-500 transition-all"
                    placeholder="Ej: Tomate Cherry" value={newWaste.ingredient} onChange={e => setNewWaste({...newWaste, ingredient: e.target.value})} />
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Cantidad</label>
                  <div className="flex gap-2">
                    <input type="number" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-rose-500 transition-all"
                      value={newWaste.amount} onChange={e => setNewWaste({...newWaste, amount: parseFloat(e.target.value) || 0})} />
                    <select className="w-24 px-4 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-rose-500 transition-all"
                      value={newWaste.unit} onChange={e => setNewWaste({...newWaste, unit: e.target.value})}>
                      <option value="kg">kg</option>
                      <option value="g">g</option>
                      <option value="l">l</option>
                      <option value="unid">unid</option>
                ...
                <div>
                  ...
                  <input type="number" step="0.01" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-rose-500 transition-all"
                    value={newWaste.cost} onChange={e => setNewWaste({...newWaste, cost: parseFloat(e.target.value) || 0})} />
                </div>
              </div>
              <button onClick={() => { const record = { id: Math.random().toString(36).substr(2, 9), ...newWaste }; setWasteRecords([record, ...wasteRecords]); showToast('Merma registrada con éxito'); setShowMermasModal(false); setNewWaste({ dishId: mockRecipes[0]?.id ?? '', date: new Date().toISOString().split('T')[0], reason: 'Caducidad', ingredient: '', amount: 0, unit: 'kg', cost: 0 }); }}
      ...
      {toast && (
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }}
          className={cn("fixed bottom-10 right-10 px-8 py-4 rounded-2xl shadow-2xl z-[100] flex items-center gap-3 border",
